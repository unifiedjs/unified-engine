...
function getConfiguration(filePath, callback) {
  ...
  if (configuration) {
    /* istanbul ignore if - only occurs if many files are
  ...
  getLocalConfiguration(directory, options, handleLocalConfiguration);
  ...
  function handleLocalConfiguration(err, localConfiguration) {
    ...
    if (!exception) {
      try {
        config = addLocalConfiguration(localConfiguration);
      } catch (err) {
        exception = err;
      }
    }
  ...
  function addLocalConfiguration(config) {
