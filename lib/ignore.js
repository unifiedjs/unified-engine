...
function check(filePath) {
  ...
  return self.patterns.reduce(reducer, filePath.split(path.sep).some(hidden));

  function reducer(ignored, pattern) {
...
function loadPatterns(callback) {
  ...
  findUp.one([self.ignoreName], self.cwd, handleOne);

  function handleOne(err, file) {
    var result;
...
function load(filePath) {
  ...
  try {
    ...
    throw new Error('Cannot read ignore file: ' + filePath + '\n' + err.message);
  }
