...
function run(options, callback) {
  ...
  if (files.length) {
    if (hasIgnore) {
      return next(new Error('Cannot accept both `files` and `ignoreName`'));
    }

    if (settings.globs.length) {
      return next(new Error('Cannot accept both `files` and `globs`'));
    }

    if (options.streamIn) {
      return next(new Error('Cannot accept both `files` and `streamIn`'));
    }
  }

  /* Process. */
  fileSetPipeline.run({files: files}, settings, next);
